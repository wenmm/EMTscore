...
```{r}
data_for_plot <- plot_data_prepare(cell_annotation_file, nnPCA_Result_multiple, merge_colname = "name")
```
...
data_for_plot <- plot_data_prepare(cell_annotation_file, nnPCA_Mscore, merge_colname = "name")
...
```{r}
# Example heatmap
plot_heatmap_function(t(geneExp), geneList_M)
...
```{r}
SCSE_Result_multiple <- Execute_SCSE_parallel(geneExp,'/home/hwen6/project/EMTscore/2025_09_24/EMTscore/vignettes/c2.all.v2025.1.Hs.symbols.gmt', cores = 20)
...
scores <- compute_Signature_score(expr_mat = geneExp, signature_file = "/home/hwen6/project/EMTscore/2025_09_24/EMTscore/vignettes/stemsig.tsv", row_name = "stemness_score")
head(scores)
```
```{r}
#signature_file <- system.file("extdata", "stemsig.tsv", package = "EMTscore")
scores <- compute_Signature_score(expr_mat = geneExp, signature_file = "/home/hwen6/project/EMTscore/2025_09_24/EMTscore/vignettes/cellular_senescence_sig.tsv", row_name = "senescence_score")
head(scores)
```

```{r}
cl1 <- Mclust(nnPCA_Result_multiple)$classification

library(RColorBrewer)

plot(nnPCA_Result_multiple, col = brewer.pal(9,"Set1")[cl1], pch=16, asp = 1)

cl2 <- kmeans(nnPCA_Result_multiple, centers = 5)$cluster

plot(nnPCA_Result_multiple, col = brewer.pal(9,"Set1")[cl2], pch=16, asp = 1)
...
df_kmeans <- data.frame(cell = rownames(cl1),
                        Kmeans = as.character(unlist(cl1$cl1)))
